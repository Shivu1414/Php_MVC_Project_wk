...
// Now delete the row
        $sql = "DELETE FROM postedjob WHERE sr='$id' ";
        if ($conndb->query($sql) === TRUE) {
        // echo "<script> confirm('Data deleted')</script>";
        ?>
        <meta http-equiv="refresh" content="0; url=http://localhost/html/Php_Task_Through_Oops/view/viewUserPostedJobs.php?id=<?php echo $sr;?> & mail=<?php echo $gmail;?>" /> 
        <?php
        } else {
